...
const collectElements = () => {
  // function will group and store references to specific elements in a single object for easier access and manipulation instead of calling document.querySelector() everytime
  return {
...
const updateTheme = (event) => {
  // function takes an 'event' object as a parameter which represents an event triggered by submitting a from
...
const filteredResults = (event) => {
  ...
  for (const book of books) {
    // loop iterates over each book in the books array.
    ...
    for (const singleGenre of book.genres) {
      ...
      if (singleGenre === filters.genre) {
        genreMatch = true;
      } // variable is set to true if the selected genre is 'any' or if the book's genre matches the selected genre.
    }
  ...
  for (const { author, id, image, title } of result.slice(0, BOOKS_PER_PAGE)) {
    // loop iterates over the filtered books, creating a button element for each one. button contains the book's image, title, and author, and has a unique data-preview attribute.
...
const bookDetails = (event) => {
  ...
  for (const node of pathArray) {
    // loop iterates over the elements in the "pathArray".
    if (active) break;
  ...
  if (active) {
    ...
    html.listSubtitle.innerText = `${authors[active.author]} (${new Date(
      active.published
    ).getFullYear()})`; // content is updated with the author's name (retrieved from the authors object) and the book's publication year.
